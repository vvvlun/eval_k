...
Usage:
    ...
    python exp2_quality_impact.py \
        --citation-dir ./data/neurips_cite_analys \
        --review-dir ./data/neurips_review_analys \
...
def load_citation_data(citation_dir: str, years: List[int]) -> Dict[str, Dict]:
    ...
    for year in years:
        # 尝试多种文件名格式
        possible_files = [
            f"citation_neurips_{year}.json",
            f"citations_neurips_{year}.json",
            f"neurips_{year}_citations.json",
        ]
...
def load_agent_data(agent_dir: str, years: List[int], model: str = "llama8b") -> Dict[str, Dict]:
    ...
    支持多种格式：
    1. paper_details 列表格式（旧格式，按年份分文件）
    2. expert_evaluations 字典格式（新格式）
    3. 单个统一文件包含所有年份数据
    
    Args:
        agent_dir: 智能体结果目录或单个文件路径
        years: 年份列表
    ...
    if agent_path.is_file():
        return _parse_agent_file(agent_path)
    
    # 情况2：是目录，先尝试查找统一文件
    if agent_path.is_dir():
        unified_files = [
            f"expert_evaluations_{model}.json",
            f"expert_evaluations.json",
            f"agent_evaluations_{model}.json",
            f"agent_evaluations.json",
            f"all_years_{model}.json",
            f"neurips_all_{model}.json",
        ]
        
        for filename in unified_files:
            filepath = agent_path / filename
            if filepath.exists():
                agent_data = _parse_agent_file(filepath)
                if agent_data:
                    return agent_data
        ...
        for year in years:
            possible_files = [
                f"neurips{year}_con_{model}.json",
                f"neurips_{year}_con_{model}.json",
                f"neurips{year}_{model}.json",
                f"neurips_{year}_{model}.json",
                f"neurips{year}_expert_{model}.json",
                f"neurips_{year}_expert_{model}.json",
                f"expert_evaluations_{year}.json",
                f"expert_evaluations_neurips_{year}.json",
            ]
            
            for filename in possible_files:
                filepath = agent_path / filename
                if filepath.exists():
                    year_data = _parse_agent_file(filepath)
                    agent_data.update(year_data)
                    break
    
    return agent_data


def _parse_agent_file(filepath: Path) -> Dict[str, Dict]:
    """
    解析单个agent数据文件，自动识别格式
    
    支持的格式：
    1. paper_details 列表格式（exp1输出）
    2. expert_evaluations 字典格式（新格式）
    3. 直接 paper_id -> evaluations 字典格式
    """
    agent_data = {}
    ...
    # 格式1: paper_details 列表（旧格式/exp1输出）
    if 'paper_details' in data:
        ...
        logger.info(f"Loaded {loaded_count} agent records from {filepath} (paper_details format)")
        return agent_data
    
    # 格式2: expert_evaluations 字典（新格式）
    if 'expert_evaluations' in data:
        expert_evaluations = data.get('expert_evaluations', {})
        for paper_id, evaluations in expert_evaluations.items():
            if not paper_id or not isinstance(evaluations, dict):
                continue
            
            parsed = _parse_evaluation_dict(evaluations)
            if parsed:
                agent_data[paper_id] = parsed
                loaded_count += 1
        
        logger.info(f"Loaded {loaded_count} agent records from {filepath} (expert_evaluations format)")
        return agent_data
    
    # 格式3: 直接是 paper_id -> evaluations 的字典
    # 排除包含元数据的文件
    meta_keys = {'experiment', 'config', 'timestamp', 'metrics', 'summary'}
    if isinstance(data, dict) and not any(k in data for k in meta_keys):
        for paper_id, evaluations in data.items():
            if not isinstance(evaluations, dict):
                continue
            
            # 检查是否包含评估维度
            if any(dim in evaluations for dim in ['novelty', 'methodology', 'clarity', 'empirical']):
                parsed = _parse_evaluation_dict(evaluations)
                if parsed:
                    agent_data[paper_id] = parsed
                    loaded_count += 1
        ...
        if loaded_count > 0:
            logger.info(f"Loaded {loaded_count} agent records from {filepath} (direct dict format)")
            return agent_data
    
    logger.warning(f"Unknown format or no valid data in {filepath}")
    return agent_data


def _parse_evaluation_dict(evaluations: Dict) -> Optional[Dict]:
    """
    解析单个论文的评估数据
    
    Args:
        evaluations: 包含 novelty, methodology, clarity, empirical 的字典
    ...
    for dimension in ['novelty', 'methodology', 'clarity', 'empirical']:
        if dimension in evaluations:
            dim_data = evaluations[dimension]
            ...
            if score is not None:
                expert_scores[dimension] = float(score)
    ...
    # 计算加权分数（简单平均）
    weighted_score = sum(scores_list) / len(scores_list)
...
def merge_data(
    ...
    for paper_id, review in review_data.items():
        ...
        paper = PaperWithImpact(
            ...
            venue=review.get('venue', 'neurips'),
...
Examples:
  ...
  python exp2_quality_impact.py \\
      --citation-dir ./data/neurips_cite_analys \\
      --review-dir ./data/neurips_review_analys \\
      --agent-dir ./data/expert_evaluations.json \\
      --output ./data/results/exp2_quality_impact.json
  ...
  python exp2_quality_impact.py \\
    ...
    parser.add_argument('--citation-dir', type=str, required=True,
                        help='引用数据目录 (包含 citation_neurips_YYYY.json)')
    parser.add_argument('--review-dir', type=str, required=True,
                        help='审稿数据目录 (包含 neurips_YYYY.json)')
    parser.add_argument('--agent-dir', type=str, default=None,
                        help='智能体结果目录或单个文件路径 (支持 expert_evaluations 格式)')
